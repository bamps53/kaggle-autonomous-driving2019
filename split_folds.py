...
def add_img_coords(df, image_size, model_scale=4):
    ...
    df['c_img'] = img_xs
    df['r_heatmap'] = (df['r_img'] * image_size[0] /
                       1355 / model_scale).astype(int)
    df['c_heatmap'] = (df['c_img'] * image_size[1] /
                       3384 / model_scale).astype(int)
    df['r_offset'] = df['r_img'] * image_size[0] / \
        1355 / model_scale - df['r_heatmap']
    df['c_offset'] = df['c_img'] * image_size[1] / \
        3384 / model_scale - df['c_heatmap']
    return df
...
def main():
    ...
    if not os.path.exists('data/folds.csv'):
        ...
        kf = KFold(n_splits=config.data.num_folds,
    ...
    expanded_df = add_img_coords(expanded_df, image_size=(
        config.data.height, config.data.width))
    expanded_df.to_csv(
        'data/img{}_df.csv'.format(config.data.height), index=False)
